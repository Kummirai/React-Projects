            ...
            <div className="d-flex align-items-center justify-content-between  border border-dark col-3 mb-5 mx-auto px-3 rounded-2">
                ...
                <div className="text-white d-flex justify-content-around p-2">
                    {pages.map((page, index=1) => (
                        <p onClick={()=>handlePageChange(page)} key={index} className={ currentPage === page ? "mb-0 btn btn-danger mx-2 border-danger" : "mb-0 btn btn-outline-danger mx-2 border-danger" }>
                            { page }
                        </p>
                    ))}
                </div>
